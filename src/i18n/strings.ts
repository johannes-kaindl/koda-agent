...
defineStrings({
  en: {
    ...
    "view.truncated": "Response cut off at the token limit — the text above may be incomplete.",
    "error.truncatedEmpty": EXPLAIN_TEXTS.tokenLimitBeforeText.en,
    "view.thoughtOnly": EXPLAIN_TEXTS.reasoningOnlyNoText.en,
    ...
    "provider.confirm.title": "Koda wants to write through another plugin",
    "provider.confirm.paths": "Affected: {0}",
    "provider.confirm.ok": "Allow",
    ...
    "view.thinkingClickOn": "click turns on",
    "view.thinkingClickOff": "click turns off",
    ...
    "picker.folder.emptyRoot": "An empty field means the whole vault — that would add every note. Pick a folder, or cancel.",
    "picker.folder.invalid": "Folder rejected: {0}",
    ...
    "settings.endpoints.thirdParty": EXPLAIN_TEXTS.apiKeyThirdParty.en.replace("{content}", "{0}"),
    ...
    "error.chatBlocked": EXPLAIN_TEXTS.corsBlocked.en,
    "settings.probe": "Test",
    ...
    "settings.model.hint.unreachable": EXPLAIN_TEXTS.endpointUnreachableKeepsModel.en,
    "settings.model.hint.no-list": EXPLAIN_TEXTS.noModelList.en,
    "settings.suppress": "Suppress thinking",
    "settings.suppress.desc": EXPLAIN_TEXTS.suppressThinkingDesc.en.replace("{feature}", "{0}"),
    "settings.folder": "Koda folder",
    "settings.folder.desc": "Koda writes freely here (memory, drafts). Everything else asks first.",
    "settings.rounds": "Max tool calls per answer",
    "settings.rounds.desc": "Stops Koda from looping between tool calls forever on a single question.",
    ...
    "settings.listRows": "Maximum listed notes",
    "settings.listRows.desc": "How many notes list_notes returns at most. Above the limit nothing disappears quietly — the first line of the answer says the list is incomplete. The same limit applies to the folders list_notes names, including the folder tree.",
    "settings.compaction": "Shorten history",
    ...
    "settings.summarize": "Summarize older replies",
    "settings.summarize.desc": "If shortening alone is not enough, Koda has the model summarize older replies. Your own messages are never summarized. Costs an extra model call — a local model can lose details while summarizing, so turn it off if you distrust it.",
    "settings.summaryLen": "Summary length (% of window)",
    "settings.summaryLen.desc": "Upper bound for the summary text.",
    "view.compaction.stage1": "History shortened — {0} tool results ({1} KB) trimmed",
    ...
    "settings.contextKeep.desc": "What you deselect or add by hand in the Context tab stays that way until you press \"Reset selection\" or start a new chat. Turn this off and every message starts from the full context again — a folder or note you just added by hand disappears again after a single message.",
    "settings.contextBudget": "Content budget per message (characters)",
    "settings.contextBudget.desc": "How much note content the modes Note, All tabs and Manual may put into one message. Everything that does not fit is cut, and the block says by how much — read_note fetches the full text. The Workspace mode is not affected: it only sends pointers. With many small notes at once, the header of each note and the truncation message may push the total 50 % higher than this budget — reduce the budget if you add many notes at one time.",
    ...
    "context.autoK": "Notes from vault-rag",
    "context.autoKDec": "Fewer notes from vault-rag",
    "context.autoKInc": "More notes from vault-rag",
    "context.vaultNeedsRag": "The Vault mode needs the vault-rag plugin.",
  ...
  de: {
    ...
    "view.truncated": "Antwort am Token-Limit abgeschnitten — der Text oben kann unvollständig sein.",
    "error.truncatedEmpty": EXPLAIN_TEXTS.tokenLimitBeforeText.de,
    "view.thoughtOnly": EXPLAIN_TEXTS.reasoningOnlyNoText.de,
    ...
    "provider.confirm.title": "Koda will über ein anderes Plugin schreiben",
    "provider.confirm.paths": "Betroffen: {0}",
    "provider.confirm.ok": "Erlauben",
    ...
    "view.thinkingClickOn": "Klick schaltet an",
    "view.thinkingClickOff": "Klick schaltet aus",
    ...
    "picker.folder.emptyRoot": "Ein leeres Feld heißt der ganze Vault — das würde jede Notiz hinzufügen. Ordner wählen oder abbrechen.",
    "picker.folder.invalid": "Ordner abgelehnt: {0}",
    ...
    "settings.endpoints.thirdParty": EXPLAIN_TEXTS.apiKeyThirdParty.de.replace("{content}", "{0}"),
    ...
    "error.chatBlocked": EXPLAIN_TEXTS.corsBlocked.de,
    ...
    "settings.model.hint.unreachable": EXPLAIN_TEXTS.endpointUnreachableKeepsModel.de,
    "settings.model.hint.no-list": EXPLAIN_TEXTS.noModelList.de,
    "settings.suppress": "Denken unterdrücken",
    "settings.suppress.desc": EXPLAIN_TEXTS.suppressThinkingDesc.de.replace("{feature}", "{0}"),
    "settings.folder": "Koda-Ordner",
    "settings.folder.desc": "Hier schreibt Koda frei (Memory, Entwürfe). Alles andere fragt vorher.",
    "settings.rounds": "Maximale Tool-Aufrufe pro Antwort",
    "settings.rounds.desc": "Verhindert, dass Koda bei einer Frage endlos zwischen Werkzeug-Aufrufen hängen bleibt.",
    ...
    "settings.listRows": "Höchstzahl aufgelisteter Notizen",
    "settings.listRows.desc": "Wie viele Notizen list_notes höchstens zurückgibt. Über der Grenze verschwindet nichts still — die erste Zeile der Antwort sagt, dass die Liste unvollständig ist. Dieselbe Grenze gilt für die Ordner, die list_notes nennt, auch im Ordnerbaum.",
    "settings.compaction": "Verlauf kürzen",
    ...
    "settings.summarize": "Ältere Antworten zusammenfassen",
    "settings.summarize.desc": "Reicht Kürzen allein nicht, lässt Koda das Modell ältere Antworten zusammenfassen. Deine eigenen Nachrichten werden nie zusammengefasst. Kostet einen zusätzlichen Modellaufruf — ein lokales Modell kann dabei Details verlieren, also ausschalten, wenn du ihm nicht traust.",
    ...
    "view.compaction.stage1": "Verlauf gekürzt — {0} Tool-Ergebnisse ({1} KB) entfernt",
    ...
    "settings.contextKeep.desc": "Was du im Kontext-Tab abwählst oder per Hand hinzufügst, bleibt so, bis du \"Auswahl zurücksetzen\" drückst oder ein neues Gespräch beginnst. Aus heißt: jede Nachricht startet wieder mit dem vollen Kontext — ein gerade von Hand hinzugefügter Ordner oder eine Notiz verschwindet dann schon nach einer einzigen Nachricht.",
    "settings.contextBudget": "Inhalts-Budget je Nachricht (Zeichen)",
    "settings.contextBudget.desc": "Wie viel Notiz-Inhalt die Modi Notiz, Alle Tabs und Manuell in eine Nachricht legen dürfen. Was nicht hineinpasst, wird gekürzt, und der Block sagt um wie viel — read_note holt den vollen Text. Der Modus Arbeitsplatz ist nicht betroffen: er schickt nur Zeiger. Bei vielen kleinen Notizen gleichzeitig können die Überschriften jeder Notiz und die Kürzungsmeldung insgesamt um etwa 50 % über dieses Budget hinausgehen — setz das Budget niedriger, wenn du viele Notizen auf einmal hinzufügst.",
    ...
    "context.autoK": "Notizen aus vault-rag",
    "context.autoKDec": "Weniger Notizen aus vault-rag",
    "context.autoKInc": "Mehr Notizen aus vault-rag",
    "context.vaultNeedsRag": "Der Modus Vault braucht das Plugin vault-rag.",
