...
- **Ten tools:** `search_notes`, `read_note`, `write_note`, `move_note`, `delete_note`,
  ...
  one call; a non-recursive listing also names the subfolders and how many notes each holds, including folders without any note, and with `depth` it shows the folder tree several levels deep in one call (folders only, each with its note count); a folder note (a note named like its folder) is marked as one, so it is not
