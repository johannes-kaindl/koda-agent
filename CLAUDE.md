...
Inhalt ist **Arbeitskontext Etappe 2b**: Kodas Arbeitskontext bekommt Volltext-Quellen statt
nur Zeiger. Zwei neue Modi —
...
Etappe), GUI-Smoke **39/39** (von 32), nach der Abschluss-Fix-Welle wiederholt.
Gate steht nach der Fix-Welle auf **696/696**.

⚠️ **Kein Rescan** — direkt nach dem Release gemessen: `releases/tags/0.14.0` → **404**,
`actions/runs` → **`total_count: 0`**. Dieselbe Signatur wie bei 0.10.1, 0.12.0 und 0.13.0:
kein Workflow-Lauf bei erreichbarer API, das Konto bleibt geflaggt. Ein Scan ohne Release
gilt als durchgefallen und nimmt das *Plugin* binnen 24 h aus der Suche. Verteilung läuft
über Forgejo und den `anysource-sideloader`-Katalog.

**Der teuerste Befund der Etappe kam vom Abschluss-Review und lag zwischen zwei Tasks:**
`collectCandidates` füllte die „schon gesehen"-Menge mit den manuellen Pfaden, **bevor** die
Breitensuche startete — und deren Vorprüfung entschied in einem, ob ein Knoten *eingesammelt*
und ob er je *expandiert* wird. Gemessen an `A→B→C` mit Tiefe 2: fügte man `B.md` von Hand
hinzu, verschwand `C.md`. Der Nutzer fügt eine Notiz hinzu und **verliert** dadurch Kontext.
Bei Tiefe 1 — dem Default — ist die Wirkung null, weshalb weder Test noch Prüfpunkt es fing:
alle laufen auf Tiefe 1. Behoben durch Trennung der zwei Fragen (`gesehen` sammelt,
`expandiert` steuert die Suche). **Wer hier etwas an der Kandidaten-Sammlung ändert, prüft
zuerst, ob er die beiden Mengen wieder vermischt.**
...
- `src/core/tools/provider.ts` — **Werkzeug-Anbieter, Wirtsseite** (Spike 2026-09-25, pure):
  Vertrag `ToolProviderApi` (`tools({lang})` synchron, `execute(name,args,{lang,confirm})`,
  Fehler als Werte), `isToolProviderApi` (Form-Prüfung inkl. `writes` je Definition),
  `mountProviderTools` (Wirt gewinnt jede Kollision, unter Anbietern der zuerst gelesene;
  Kollisionen werden gemeldet, nie überschrieben), `outcomeFromProvider`. `toolSet()` in
  `defs.ts` ist die eine Menge für Draht UND Route; `src/obsidian/providers.ts` liest alle
  Anbieter aus `app.plugins.plugins[*].api` (keine Plugin-Liste, sortiert nach Id, frisch je
  Aufruf). Der Runner routet nach der Route, nicht nach dem Namen — sonst ginge ein montiertes
  `related_notes` in Kodas eigenen Pfad (Praxistest-Befund). Vertragsstand: vault-rag
  `feat/tool-provider` 92c6c4d; Festlegungen im Cockpit-Task „Werkzeug-Anbieter-Vertrag".
- `src/core/tools/list.ts` — `list_notes`: Ordnerauswahl (flach/rekursiv), Kappung
  mit Warnung in Zeile 1, leerer Ordner als Fehler mit Vorschlägen (pure). Seit 2026-09-26 dazu
  `src/core/tools/tree.ts`: der Ordnerbaum für `depth` >= 2, gekappt nach ganzen Ebenen (nie ein
  halber Ast — der sähe aus wie ein fehlender Ordner), Kappung ebenfalls in Zeile 1. Frontmatter
